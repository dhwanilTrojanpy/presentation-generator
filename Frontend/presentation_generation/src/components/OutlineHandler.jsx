...
function OutlineHandler({setOutlines,outlines,isGenerating}) {
    ...
    const handleEdit = (index) => {
      ...
      const handleSaveOutlines = (outlines) => {
          ...
          axios.post("http://localhost:8000/save-outline",{
            outlines : outlines
          } )
      }
    return (
          ...
          {outlines.length > 0 ? (
            <div className="outline-container">
                ...
                {outlines.map((outline, index) => (
                  <li type="text"key={index} className="outline-item">
                   <div className="outline-edit-container">
                      <div className="outline-edit" onBlur ={(e)=> handlenewOutline(e,index) } 
                      contentEditable={EditingIndex === index}
                      >
                    {outline}
                        </div>
                    <button type="button" className="btn btn-sm btn-primary" onClick={() => handleEdit(index)}>
              ...
              <div className="outline-actions">
                <button className="action-btn" onClick={() => {handleSaveOutlines(outlines)}}>Save Outline</button>
              </div>
            </div>
          ) : (
